...
class PaymentTransaction(models.Model):
    ...
    def _send_payment_request(self):
        ...
        if not self.payment_token_id:
            raise UserError("Adyen Direct: " + _("The transaction is not linked to a token."))
        ...
        data = {
            ...
            'shopperReference': self.payment_token_id.adyen_shopper_reference,
        ...
        self._handle_feedback_data('adyen_direct', response_content)
    ...
    def _get_tx_from_feedback_data(self, provider, data):
        ...
        if provider != 'adyen_direct':
            return tx

        reference = data.get('merchantReference')
        if not reference:
            raise ValidationError("Adyen Direct: " + _("Received data with missing merchant reference"))
        event_code = data.get('eventCode')

        tx = self.search([('reference', '=', reference), ('provider', '=', 'adyen_direct')])
        if event_code == 'REFUND' and (not tx or tx.operation != 'refund'):
            ...
            source_tx = self.search(
                [('acquirer_reference', '=', source_acquirer_reference), ('provider', '=', 'adyen_direct')]
            )
            if source_tx:
        ...
        if not tx:
            raise ValidationError(
                "Adyen Direct: " + _("No transaction found matching reference %s.", reference)
            )
        return tx
    ...
    def _adyen_create_refund_tx_from_feedback_data(self, source_tx, data):
        ...
        if not refund_acquirer_reference or not amount_to_refund:
            raise ValidationError(
                "Adyen Direct: " + _("Received refund data with missing transaction values")
            )
    ...
    def _process_feedback_data(self, data):
        ...
        if not payment_state:
            raise ValidationError("Adyen Direct: " + _("Received data with missing payment state."))
        ...
        if payment_state in RESULT_CODES_MAPPING['pending']:
            ...
            if self.operation == 'refund':
                ...
                acquirer_id._cron_post_process_after_done()
    ...
    def _adyen_tokenize_from_feedback_data(self, data):
        ...
        _logger.info(
            "created token with id %s for partner with id %s", payment_token.id, self.partner_id.id
        )
