...
class PaymentAcquirer(models.Model):
    ...
    def _adyen_direct_make_request(
        ...
        try:
            ...
            raise ValidationError("Adyen Direct: " + _("Could not establish the connection to the API."))
        except requests.exceptions.HTTPError as error:
            _logger.exception(
                "invalid API request at %s with data %s: %s", url, payload, error.response.text
            )
            raise ValidationError("Adyen Direct: " + _("The communication with the API failed."))
