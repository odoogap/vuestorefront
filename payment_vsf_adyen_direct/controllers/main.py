...
class AdyenDirectController(http.Controller):
    ...
    @http.route('/payment/adyen_direct/acquirer_info', type='json', auth='public')
    ...
    @http.route('/payment/adyen_direct/payment_methods', type='json', auth='public')
    ...
    @http.route('/payment/adyen_direct/payments', type='json', auth='public')
    def adyen_payments(
        ...
        request.env['payment.transaction'].sudo()._handle_feedback_data(
            'adyen_direct', dict(response_content, merchantReference=reference),  # Match the transaction
        )
        return response_content

    @http.route('/payment/adyen_direct/payment_details', type='json', auth='public')
    ...
    @http.route('/payment/adyen_direct/return', type='http', auth='public', csrf=False, save_session=False)
    def adyen_return_from_redirect(self, **data):
        ...
        if payment_transaction.vsf_pay_by_link:
            vsf_payment_success_return_url = website.vsf_pay_url
            vsf_payment_error_return_url = sale_order.get_portal_url()
        ...
        _logger.info("handling redirection from Adyen Direct with data:\n%s", pprint.pformat(data))
